...
public class RoutePayment extends Metric{
	//Parameters:
	protected int totalTime = 10000; //changed it from 1000 so that the forcibal awakening of src and dst doesn't
									 //impact the outcome that much
	protected int endTime = 10000;   //same here
	...
	protected double nodeSleep = 0; // <-- newly added
	protected double nodeWakeUp = 0; // <-- newly added
	protected List<List<int[]>> boomPaths = new ArrayList<>();

	/**
	 * Added 2 Constructors for nodeSleep and nodeWakeUp
	 * */
	public RoutePayment(PathSelection ps, int trials, boolean up,double redundancy, double nodeSleep, double nodeWakeUp) {
		this(ps,trials,up,Integer.MAX_VALUE,redundancy, nodeSleep, nodeWakeUp);
	}
	 ...
	 * @param epoch
	 */
	public RoutePayment(PathSelection ps, int trials, boolean up, int epoch, double redundancy, double nodeSleep, double nodeWakeUp) {
		...
		this.nodeSleep = nodeSleep; // added nodeSleep and nodeWake
		this.nodeWakeUp = nodeWakeUp;
	}

	public RoutePayment(PathSelection ps, int trials, boolean up,double redundancy) {
		this(ps,trials,up,Integer.MAX_VALUE,redundancy); // added nodeSleep and nodeWakeUp
	}
	...
	public static Parameter[] extendParams(String selName, int trials, boolean up, String distName, int epoch, Parameter[] params) {
		...
		System.arraycopy(params, 0, nparams, 5, params.length);
		return nparams;
	}
...
/*			if (!dealt.contains(node)) {
	...
	public void computeData2(Graph g, Network n, HashMap<String, Metric> m) {
		...
		for (int i = 0; i < this.transactions.length; i++) {
			...
			if(tempSum<vals[i]*(1+totalFeeRate)) //If the potentialSum of all the outgoing nodes (who are closer to the
												 // dst) is smaller than the payment, then the transaction aborts
			{
				uselessCnt++;
				continue;
			}
			...
			if(tempSum<vals[i]*(1+totalFeeRate)) // Same here as above
			{
				uselessCnt++;
				continue;
			}
		...
		/** newly added
		 ...
		 offline array represents the sleeping nodes, which are excluded as possible intermediaries
		 The average amount of nodes, based on the constants nodeSleep and nodeWakeup, are put to sleep.

		 **/
		Boolean[] offline = new Boolean[nodes.length];

		if (nodeWakeUp>0) {
			Random rand = new Random();
			//average amount of nodes sleeping
			int avgSleepingNodes = (int) (nodes.length * (nodeSleep/(nodeSleep+nodeWakeUp)));
			List<Integer> nodeIndex = IntStream.range(0, nodes.length).boxed().collect(Collectors.toList());
			for (int i = 0; i < avgSleepingNodes; i++) { //avgSleepingNodes amount of nodes are put to sleep
				int randomIndex = rand.nextInt(nodeIndex.size());
				offline[nodeIndex.get(randomIndex)] = true;
				nodeIndex.remove(randomIndex);
			}
		}
		...
		for (int i = 0; i < endTime; i++) {
			...
			//newly added, checks if nodes fall asleep/wake up
			//checks every second once
			for (int l = 0; l < offline.length; l++) {
			...
			for (int ii = 0; ii < this.timeQueue[i].size(); ii++) {
				...
				if (!pps.isEmpty() && hopCount[id] < maxHopCount[id]) {
					...
					for (int j = 0; j < pps.size(); j++) {
						...
						//newly added
						for (int l = 0; l < excluded.length; l++) {
							if (offline[l]!= null && offline[l]) {
								excluded[l] = true;
							}
						}
						offline[dsts[id]] = false; //newly added, the dst is woken up
			...
			timeQueue[i].clear();
			//System.out.println("");
			if(i%tInterval==tInterval-1&& intervalCount &&i<totalTime) {
				int cnt = 0;
		...
		//List<Boolean> asList = Arrays.asList(offline);
		//System.out.println(Collections.frequency(asList,true));
