...
public class PaymentTests {
	...
	public static void runSimpleTestSynthetic() {
		/** Implementation of sleeping nodes **/
		for(int i=1;i<4;i++) {
			...
			for (int epoch = 1; epoch < 9; epoch++) {
				...
				Transformation[] trans = new Transformation[]{new InitCapacities(200, -1, BalDist.EXP),
						...
						new Transactions(50*epoch+0.001*i, -1, TransDist.EXP, false, 100000, true, false)
				...
				Metric[] m = new Metric[]{
						...
						// SpeedyMurmurs
						//new RoutePayment(new SplitIfNecessary(hop), trials, up, 0), // split if necessary, HopDistance
						//new RoutePayment(new SplitIfNecessary(speedyMulti), trials, up, 1.33),
						...
						//new RoutePayment(new SplitAndAdjustSize(speedyMulti), trials, up, 1.1), //FUFi1.1
						/**
						 * For avg sleep 0: nodesleep = 0;
						 * For avg sleep 1/10: nodeSleep = 0.001 nodeWakeUp=0.009;
						 * For avg sleep 1/4: nodesleep = 0.003 nodeWakeUp=0.009;
						 * For avg sleep 1/2: nodesleep = 0.01 nodeWakeUp=0.01;
						 * For avg sleep 3/4: nodesleep = 0.009 nodeWakeUp=0.003;
						 * */
						new RoutePayment(new SplitAndAdjustSize(speedyMulti), trials, up, 1.4, 0.009, 0.003), //FUFi1.4
						//new RoutePayment(new SplitAndAdjustSize(speedyMulti), trials, up, 1.8), //FUFi1.8
						//new RoutePayment(new SplitAndAdjustSize(speedyMulti), trials, up, 2.4+0.001*epoch),
						//new RoutePayment(new ForwardDirect(speedyMulti), trials, up,1.0+0.001*epoch),
						//new RoutePayment(new ForwardDirect(speedyMulti), trials, up, 1.33),
						//new RoutePayment(new ForwardDirect(speedyMulti), trials, up, 1.81+0.001*epoch),
						//new RoutePayment(new SplitClosest(speedyMulti), trials, up, 0), //SplitClosest
